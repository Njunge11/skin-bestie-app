...
describe("Step 3: Skin Concerns - User Workflows", () => {
  ...
  it("user encounters validation error when no concern selected and recovers", async () => {
    ...
    expect(
      await screen.findByText(/pick at least one concern/i),
    ).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(
        screen.queryByText(/pick at least one concern/i),
  ...
  it('user selects "Other" and text field appears with validation', async () => {
    ...
    expect(
      screen.queryByPlaceholderText(/write something/i),
    ).not.toBeInTheDocument();
    ...
    expect(
      await screen.findByText(/please describe "other"/i),
    ).toBeInTheDocument();

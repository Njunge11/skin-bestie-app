...
//     <>
//       <header
//         className={`fixed top-0 left-0 right-0 z-30 transition-all duration-300 py-3 sm:py-4 lg:py-5 bg-[#13110F]/30 ${
...
const Header = () => {
  ...
  return (
    <>
      {/* Inside the pinned scene; backdrop blur on inner overlay */}
      <header className="relative z-30 py-3 sm:py-4 lg:py-5">
        <div className="absolute inset-0 -z-10 backdrop-blur-md bg-[#13110F]/30" />
        <div className="w-full px-4 sm:px-6 lg:px-8 xl:max-w-7xl xl:mx-auto">
          <nav className="flex items-center justify-between">
            <a href="/" className="block">
              <img
                src="/skinbestie-logo.png"
                alt="Logo"
                className="h-8 sm:h-10 lg:h-[42px] w-auto"
              />
            </a>
            ...
            <div className="hidden xl:flex items-center gap-6">
              {links.map((l) => (
                <a
                  key={l.href}
                  href={l.href}
                  className="text-[#FAFAFA] font-semibold text-sm uppercase hover:text-gray-200"
                >
                  {l.label}
                </a>
              ))}
            ...
            <div className="hidden xl:block">
              <button className="text-[#FAFAFA] rounded-xl px-4 py-3 bg-white/5 border border-[#FDFAEB]/30 hover:bg-white/10">
                Sign into your account
              </button>
            </div>
            ...
            <button
              onClick={() => setOpen((v) => !v)}
              className="xl:hidden w-10 h-10 rounded-lg hover:bg-white/10 flex items-center justify-center"
            >
      ...
      {open && (
        <div className="xl:hidden absolute inset-0 z-[70] bg-[#FFFBE7]">
          <div className="absolute top-0 left-0 right-0 py-4 px-4 sm:px-6 flex items-center justify-between">
            <img
              src="/skinbestie-logo-dark.png"
              alt="Logo"
            ...
            <div className="w-full max-w-md space-y-8">
              {links.map((l) => (
                <a
                  ...
                  href={l.href}
                  onClick={() => setOpen(false)}
                  className="block text-[#13110F] text-2xl sm:text-3xl text-center hover:text-gray-700"
                >
                  {l.label}
                </a>
              ))}
              <div className="pt-8">
                <button
                  onClick={() => setOpen(false)}
                  className="w-full rounded-xl p-4 border border-[#13110F]/30 hover:bg-[#13110F]/5"
                >
                  Sign into your account
                </button>
              </div>
